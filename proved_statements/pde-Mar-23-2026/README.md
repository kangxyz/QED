...
The proof is human-verified by Qiao Zhuang, Qihao Ye, and Zhongqiang Zhang.

## Workflow

- Experts gave QED the problem: finding a function that satisfies the constraints.
- QED returned a candidate with proof.
- Experts found that the constraints were too weak (they had initially given the wrong problem).
- Experts strengthened the constraints, gave the problem back to QED, and instructed QED to continue the search starting from the previous candidate.
- QED returned the final candidate and the proofs.
- In the entire loop, there was no human intervention besides the experts changing their problem statement once.

## Expert Comments from Qiao Zhuang, Qihao Ye, and Zhongqiang Zhang
...
Will be released after the paper is arxived.
